...
public abstract class AbstractUVCCameraHandler extends Handler {
    ...
    public interface OnEncodeResultListener {
        ...
        //增加关键帧标识 0 不是关键帧  1 关键帧
        void onEncodeResult(byte[] data, int offset, int length, long timestamp, int type,int keyFrame);
...
//            throw new IllegalArgumentException("surface should be one of SurfaceHolder, Surface or SurfaceTexture: " + surface);
    ...
    // 开始录制
    public void startRecording(Context context,final RecordParams params, OnEncodeResultListener listener) {
    ...
    public void startRecording(RecordParams params) {
        checkReleased();
        sendMessage(obtainMessage(MSG_CAPTURE_START, params));
    }
    // 停止录制
...
//			thread.handleStartRecording((RecordParams) msg.obj);
                thread.handleStartPusher(context,(RecordParams) msg.obj);
                break;
...
//		private int mSoundId;
        ...
        private UVCCamera mUVCCamera;
        		...
        		private MediaMuxerWrapper mMuxerwarp;
...
//            handleStopRecording();
        ...
        public void handleStartPreview(final Object surface) {
            ...
            try {
                mUVCCamera.setPreviewSize(mWidth, mHeight, UVCCamera.DEFAULT_PREVIEW_MIN_FPS, UVCCamera.DEFAULT_PREVIEW_MAX_FPS, mPreviewMode, mBandwidthFactor);
...
//				mUVCCamera.setFrameCallback(mIFrameCallback, UVCCamera.PIXEL_FORMAT_NV21);
                ...
                try {
                    ...
                    mUVCCamera.setPreviewSize(mWidth, mHeight, UVCCamera.DEFAULT_PREVIEW_MIN_FPS, UVCCamera.DEFAULT_PREVIEW_MAX_FPS, UVCCamera.DEFAULT_PREVIEW_MODE, mBandwidthFactor);
        ...
        // 捕获静态图片
        public void handleCaptureStill(int id,final String path) {
...
//			mSoundPool.play(mSoundId, 0.2f, 0.2f, 0, 0, 1.0f);	// play shutter sound
            try {
                ...
                final File outputFile = TextUtils.isEmpty(path)
                        ? MediaMuxerWrapper.getCaptureFile( id,".jpg")
                        : new File(path);
		...
		public void handleStartRecording(RecordParams params) {
            ...
            try {
                if ((mUVCCamera == null) || (mMuxerwarp != null)) return;
                final MediaMuxerWrapper muxer = new MediaMuxerWrapper(params.getId(),".mp4");	// if you record audio only, ".m4a" is also OK.
        ...
        public void handleStartPusher(Context context,RecordParams params) {
...
//                mMuxer = new Mp4MediaMuxer(context,params.getId(),60 * 1000, params.isVoiceClose());
//
//            }
            // 启动视频编码线程
            startVideoRecord();
        ...
        private void startVideoRecord() {
            ...
            mH264Consumer.setOnH264EncodeResultListener(new H264EncodeConsumer.OnH264EncodeResultListener() {
                ...
                public void onEncodeResult(byte[] data, int offset, int length, long timestamp,int keyFrame) {
                    if (mListener != null) {
                        mListener.onEncodeResult(data, offset, length, timestamp, 1 , keyFrame);
                    }
                }
            });
            mH264Consumer.start();
...
//                    if (outData == null) {
//                        outData = new byte[yuvData.length];
//                    }
//                    String date = mFormat.format(new Date());
//                    YuvOsdUtils.addOsd(yuvData, outData, date);
