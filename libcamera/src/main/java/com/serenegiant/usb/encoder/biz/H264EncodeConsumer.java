...
//    private Mp4MediaMuxer mMuxer;
    public interface OnH264EncodeResultListener {
        void onEncodeResult(byte[] data, int offset,
                            int length, long timestamp,int keyFrame);
    }
    ...
    public synchronized void start(Mp4MediaMuxer mMuxer) {
//        this.mMuxer = mMuxer;
        start();
    }
...
//                nv21ToNV12(yuvData, UVCCamera.DEFAULT_PREVIEW_WIDTH, UVCCamera.DEFAULT_PREVIEW_HEIGHT);
                feedMediaCodecData(outData);
    ...
    private void feedMediaCodecData(byte[] data) {
        ...
        try {
            ...
            Log.d(TAG, "feedMediaCodecData throw IllegalStateException");
        }
        if (bufferIndex >= 0) {
            ByteBuffer buffer;
    ...
    public void run() {
        ...
        while (!isExit) {
            try {
                ...
                do {
                    outputBufferIndex = mMediaCodec.dequeueOutputBuffer(bufferInfo, TIMES_OUT);
...
//                        }
                        ...
                        if (sync) {
                            ...
                            if (listener != null) {
                                listener.onEncodeResult(h264, 0, mPpsSps.length + bufferInfo.size, bufferInfo.presentationTimeUs / 1000, 1);
                            }
...
//                        }
                            ...
                            if (listener != null) {
                                listener.onEncodeResult(h264, 0, bufferInfo.size, bufferInfo.presentationTimeUs / 1000, 0);
                            }
                            // 添加视频流到混合器
...
//                        }
    ...
    private void startMediaCodec() {
        ...
        YuvOsdUtils.initOsd(10, 10, pattern.length(), mWidth, mHeight, 0);
