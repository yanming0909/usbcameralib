...
//    }
    ...
    public void initUSBMonitor(Context activity, final CameraViewInterface cameraView, final OnMyDevConnectListener listener) {
        ...
        mUSBMonitor = new USBMonitor(activity.getApplicationContext(), new USBMonitor.OnDeviceConnectListener() {
            ...
            public void onAttach(UsbDevice device) {
                if (device.getVendorId()==1060||device.getProductId()==60416)return;
                if (listener != null) {
...
//        openCamera(mCtrlBlock);
    ...
    public void requestPermission(final int index) {
        final List<UsbDevice> devList = getUsbDeviceList();
        if (devList == null || devList.size() == 0) {
            return ;
        }
        int count = devList.size();
        Log.d(TAG, "devList.size(): "+count);

        if (index < count) {
            if (count >=2) {
                UsbDevice device = devList.get(0);
                if (device.getVendorId()==11491) Collections.swap(devList, 0, 1);
//                if (usbDevice!=null&& device.getVendorId() == usbDevice.getVendorId())
//                    Collections.swap(devList, 0, 1);
            }
            if (mUSBMonitor != null) {
                new Handler().postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        Log.d(TAG, "requestPermission"+index+": "+devList.get(index).getVendorId());
    ...
    public void requestPermission(final UsbDevice device , long delay) {
            if (mUSBMonitor != null) {
                new Handler().postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        Log.d(TAG, "requestPermission"+"--> VID:"+device.getVendorId()+" | PID:"+device.getProductId());
    ...
    public void startPusher(Context context,AbstractUVCCameraHandler.OnEncodeResultListener listener) {
        if (mCameraHandler != null && !isPushing()) {
            mCameraHandler.startRecording(context,null, listener);
        }
    }
    public void startPusher(RecordParams params) {
        if (mCameraHandler != null) {
            mCameraHandler.startRecording(params);
        }
    }
    public void startPusher(Context context,RecordParams params, AbstractUVCCameraHandler.OnEncodeResultListener listener) {
        if (mCameraHandler != null && !isPushing()) {
            mCameraHandler.startRecording(context,params, listener);
        }
    }
    ...
    public void setOnPreviewFrameListener(AbstractUVCCameraHandler.OnPreViewResultListener listener) {
        if(mCameraHandler != null) {
            mCameraHandler.setOnPreviewFrameListener(listener);
        }
    }
    ...
    public void startPreview(CameraViewInterface view) {
//        SurfaceTexture st = view.getSurfaceTexture();
//        if (mCameraHandler != null) {
//            mCameraHandler.startPreview(st);
//        }
         Surface st = view.getSurface();
        if (mCameraHandler != null) {
            mCameraHandler.startPreview(st);
        }
    }
    ...
    public int getPreviewWidth() {
        return mCameraHandler.getWidth();
    }

    public int getPreviewHeight() {
        return mCameraHandler.getHeight();
    }
}
